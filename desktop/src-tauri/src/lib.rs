...
// OBF_KEY is generated by build.rs from the AETHER_CLIENT_KEY build secret - it
// is NOT in the public source. Empty in community/dev builds.
include!(concat!(env!("OUT_DIR"), "/client_secret.rs"));
fn deobf(b: &[u8]) -> String {
    String::from_utf8(b.iter().enumerate().map(|(i, c)| c ^ OBF_XORK[i % OBF_XORK.len()]).collect())
        .unwrap_or_default()
}
/// Built-in signed-feed endpoint (deobfuscated in memory).
fn feed_url() -> String {
    deobf(OBF_URL)
}
/// Shared client key sent as `x-aether-key` so only AetherAV clients can pull.
...
fn update_now(app: tauri::AppHandle, state: tauri::State<AppState>) -> Value {
    ...
    // Identify as a real client so the server's gate lets us pull.
    let bytes = match http_bytes(&url, Some(&client_key())) {
        Ok(b) => b,
