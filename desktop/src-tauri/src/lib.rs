...
// Obfuscated server endpoint + shared client key. This is NOT a substitute for
// real auth (the feed is Ed25519-signed and the server independently gates
// `/feed` on this key) - it just keeps the URL/key out of a trivial `strings`
// dump of the binary. Decoded only in memory, at call time.
const OBF_XORK: &[u8] = &[0x9e, 0x4d, 0x21, 0xb7, 0x6c, 0xf3, 0x08, 0x5a];
const OBF_URL: &[u8] = &[0xf6,0x39,0x55,0xc7,0x1f,0xc9,0x27,0x75,0xff,0x28,0x55,0xdf,0x09,0x81,0x25,0x39,0xfb,0x23,0x55,0xc5,0x0d,0x9f,0x26,0x3b,0xed,0x3a,0x52,0xc4,0x42,0x90,0x67,0x37,0xb1,0x2b,0x44,0xd2,0x08];
fn deobf(b: &[u8]) -> String {
    String::from_utf8(b.iter().enumerate().map(|(i, c)| c ^ OBF_XORK[i % OBF_XORK.len()]).collect())
        .unwrap_or_default()
}
/// Built-in signed-feed endpoint (deobfuscated in memory; not a secret, just kept
/// out of a trivial `strings` dump).
fn feed_url() -> String {
    deobf(OBF_URL)
}

/// Per-device Ed25519 identity. Auto-generated on first run and stored locally;
/// the private seed never leaves the machine and NO shared secret exists anywhere
/// (nothing in the repo or binary). Each client proves itself by signing a fresh
/// timestamped request, which the server verifies against the advertised pubkey.
fn device_seed(assets: &Path) -> [u8; 32] {
    let path = assets.join("models/device.key");
...
impl Default for Settings {
    fn default() -> Self {
        Settings {
            ...
            auto_update_hours: 6,
            // Empty -> the client uses its built-in (obfuscated) feed endpoint.
            // Power users can still set an explicit URL here to override it.
...
fn http_bytes(url: &str, headers: &[(&str, &str)]) -> Result<Vec<u8>, String> {
    use std::io::Read;
